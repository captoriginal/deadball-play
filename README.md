...
Web-assisted generation requires `./run_dev.sh` to be running. The start screen
lists MLB games by date and exposes the Web generator's Standard, SABR, and
Adaptive trait modes plus optional statistics refresh and score-sheet side
selection. Both home and away PDFs are generated by default. Generation always
downloads the command-safe game JSON and selected PDF score sheets together
into `saves/`; live session saves remain there for now as `.save.json` files.
